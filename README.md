...
[![Build iOS ipa](https://github.com/isy-team/isy/actions/workflows/build-ios.yml/badge.svg)](https://github.com/isy-team/isy/actions/workflows/build-ios.yml)
...
| 测试覆盖 | 32 个单元测试 (ELF 解析 / patcher 算法 / syscall 分发) |
...
│   │   ├── Emulator/          CPUState + Memory + Emulator
│   │   ├── Syscall/           SyscallTable + Dispatcher + File/Signal/Network/Process syscalls
│   │   ├── Process/           LinuxProcess + ProcessManager
│   │   └── FS/                RootFS + VirtualFS
│   ├── isyUI/                 SwiftUI Terminal (跨平台, 非 SwiftUI 平台编译为空)
│   ├── isyApp/                iOS App 入口
│   └── isycli/                命令行宿主 (info/parse/patch/bench/syscall)
└── Tests/isyCoreTests/        3 个测试套件
```
...
| 优先级 | 模块 | 状态 |
|---|---|---|
| P0 | ELF 加载 + binary patching | ✅ 完成 |
| P0 | 基础 syscall (read/write/open/exit/...) | ✅ 完成 |
| P0 | 信号 / 网络 / 文件操作 syscall | ✅ 骨架完成 |
| P0 | 动态链接器 (.so 重定位) | 🔨 骨架完成 |
| P0 | rootfs 打包 (Alpine ARM64) | 🔨 框架完成 |
| P1 | 真实模式 stdio 桥接 (跑通 `ls`) | 🔨 框架完成 |
| P1 | clone/fork (GCD 线程模拟) | 📋 TODO |
| P2 | OverlayFS 完整实现 | 📋 TODO |
